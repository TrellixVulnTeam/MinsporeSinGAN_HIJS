...
class Generator(nn.Cell):
    def __init__(self, img_size_min, num_scale, scale_factor=4/3):
        ...
        first_generator.append(nn.SequentialCell(nn.Conv2d(3, self.nf, 3, 1),
                                                 nn.BatchNorm2d(self.nf),
                                                 nn.LeakyReLU(2e-1)))
        for _ in range(3):
            first_generator.append(nn.SequentialCell(nn.Conv2d(self.nf, self.nf, 3, 1),
                                                     nn.BatchNorm2d(self.nf),
                                                     nn.LeakyReLU(2e-1)))

        first_generator.append(nn.SequentialCell(nn.Conv2d(self.nf, 3, 3, 1),
                                                 nn.Tanh()))
