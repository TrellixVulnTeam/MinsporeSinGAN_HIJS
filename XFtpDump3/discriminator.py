...
class Discriminator(nn.Cell):
    def __init__(self):
        ...
        first_discriminator.append(nn.SequentialCell(
            [nn.Conv2d(self.nf, 1, 3, 1, padding=1, has_bias=True, pad_mode='pad')]))
    ...
    def progress(self):
        ...
        if self.current_scale % 4 != 0:
            ...
            if self.current_scale >= 1:
                tmp_discriminator = mindspore.load_param_into_net(
                    tmp_discriminator, prev_discriminator.parameters_dict)  # 以python的字典格式加载存储
