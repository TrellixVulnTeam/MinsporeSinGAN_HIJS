...
def maxminnorm(array):
    ...
    data_s0 = data_shape[0]
    data_s1 = data_shape[1]
    data_s2 = data_shape[2]
    data_s3 = data_shape[3]
    t = np.empty((data_s0, data_s1, data_s2, data_s3))
    for j0 in range(data_s0):
        for j1 in range(data_s1):
            for j2 in range(data_s2):
                for j3 in range(data_s3):
                    t[j0, j1, j2, j3] = (
                        array[j0, j1, j2, j3]-mincols)/(maxcols-mincols)
    return t
